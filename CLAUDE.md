...
> 通則：以下四原則偏向「謹慎優先於速度」。瑣碎改動可自行判斷，
> 但涉及 RLS／權限／schema／AI route 一律從嚴。

### 1. 先想再寫
- 明列假設；不確定就先問，不要默默選一種解讀。
- 有多種詮釋時全部攤開，不自行拍板。
- 有更簡單的做法直接講，該 push back 就 push back。

### 2. 最簡實作
- 只寫解決問題的最小程式，不做沒被要求的功能。
- 單次使用的程式不抽象化；沒被要求的彈性／可設定性不加。
- 不為不可能發生的情境寫 error handling。
- 寫了 200 行但 50 行能解決，就重寫。

### 3. 外科式修改
- 只動非動不可的部分，不順手「改善」相鄰程式／註解／排版。
- 沒壞的不重構；對齊既有風格，即使你會用別的寫法。
- 看到無關 dead code → 講出來，不要刪（呼應 Git Workflow：既有 migration 編號衝突不回頭改名）。
- 只清理你自己改動造成的孤兒 import／變數／函式。
- 檢驗：每一行改動都能直接對應到需求。

### 4. 目標導向驗證
- 把任務轉成可驗證的成功條件，再 loop 到通過：「加驗證」→ 定義無效輸入的預期行為並做到；「修 bug」→ 先穩定重現再修到不再重現；「重構」→ 改動前後行為一致。
- ⚠️ 本 repo 目前無自動化測試框架，「驗證」即既有發布前流程：`npm run typecheck` + `npm run build` 皆綠燈，涉及權限時手動驗 `can_access_deal()` 三層；未來導入測試框架後，bug 修復優先 test-first。
- 多步驟任務先給簡短計畫（步驟 → 驗證點）。
