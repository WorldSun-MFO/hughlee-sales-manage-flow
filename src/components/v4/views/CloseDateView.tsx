...
// 每列由左到右:tier / 客戶名稱 / 業務名 / 產品名 / AUM / 目標成交日,
// 點整列超連結到該客戶詳情頁。逾期 / 7 天內以顏色標示,方便管理員逐一確認。
...
const GRID = 'sm:grid-cols-[52px_minmax(0,1.6fr)_minmax(0,1fr)_minmax(0,1fr)_minmax(0,0.9fr)_minmax(0,1.2fr)]';
...
export function CloseDateView({ deals, base }: { deals: LightDeal[]; base: '/workspace' | '/hub' }) {
  const rows = useMemo(() => {
    ...
    return deals
      ...
      // 'YYYY-MM-DD' 字串字典序 = 時間序,直接排即由近到遠(逾期者最前)
      .sort((a, b) => (a.target_close_date! < b.target_close_date! ? -1 : a.target_close_date! > b.target_close_date! ? 1 : 0))
      .map((d) => ({ deal: d, info: closeDateInfo(d.target_close_date!, todayMs) }));
  }, [deals]);
      ...
      {rows.length === 0 ? (
        ...
        <section className="grid gap-2">
          ...
          <ul className="grid gap-2">
            {rows.map(({ deal: d, info }) => {
              const accent = info.tone === 'overdue' ? 'border-l-claret bg-claret/4'
                : info.tone === 'soon' ? 'border-l-brass bg-brass/4'
                  : 'border-l-ink/20 bg-paper';
              const dateTone = info.tone === 'overdue' ? 'text-claret'
                : info.tone === 'soon' ? 'text-brass' : 'text-ink/65';
              return (
                <li key={d.id}>
                  <Link
                    href={`${base}/clients/${d.id}` as never}
                    className={`group grid grid-cols-[44px_1fr_auto] items-center gap-3 rounded-md border border-ink/10 border-l-4 p-4 transition hover:border-ink/30 hover:shadow-panel ${GRID} ${accent}`}
                  >
                    {/* tier */}
                    <span className={`grid h-7 w-9 place-items-center rounded-sm font-v4-mono text-[11px] font-bold ${d.tier ? TIER_STYLES[d.tier] : 'bg-ink/10 text-ink/45'}`}>
                      {d.tier ?? '—'}
                    </span>
                    ...
                    {/* 業務名(桌機) */}
                    <div className="hidden min-w-0 truncate text-sm text-ink/75 sm:block">{d.rm?.full_name ?? '—'}</div>
                    {/* 產品名(桌機) */}
                    <div className="hidden min-w-0 truncate text-sm text-ink/75 sm:block">{d.product ?? '—'}</div>
                    {/* AUM(桌機) */}
                    <div className="numeric hidden font-v4-mono text-sm font-semibold text-ink sm:block">{fmtMoney(Number(d.aum_usd))}</div>

                    {/* 目標成交日 */}
                    <div className="flex items-center justify-end gap-2 sm:justify-start">
                      <div className="text-right sm:text-left">
                        <div className={`numeric font-v4-mono text-sm font-semibold ${dateTone}`}>{info.label}</div>
                        <div className={`font-v4-mono text-[11px] ${dateTone}`}>{info.countdown}</div>
                      </div>
                      <ArrowUpRight className="h-4 w-4 shrink-0 text-ink/25 transition group-hover:text-ink sm:hidden" strokeWidth={1.75} />
                    </div>
                  </Link>
                </li>
              );
