// /v4/hub/clients/[id] — 客戶詳情(streaming)。同 workspace 版,只是外面套 HubTopBar
...
export default async function HubClientDetailPage(
  ...
  if (!deal) {
    return (
      <>
        <HubTopBar pageLabel="找不到客戶" source={isFixtures ? 'fixtures' : 'supabase'} />
        <div className="mx-auto max-w-[1240px] grid place-items-center px-8 py-20">
          <div className="grid gap-3 text-center">
            ...
            <Link href="/v4/hub/clients" className="inline-flex items-center gap-1.5 font-v4-mono text-xs font-semibold text-ink/55 hover:text-ink">
  ...
  return (
    <>
      <HubTopBar pageLabel={deal.name.replace(/^【範例】/, '')} source={isFixtures ? 'fixtures' : 'supabase'} />
      <div className="mx-auto max-w-[1240px]">
        <ClientDetailShell deal={deal} backHref="/v4/hub/clients" isFixtures={isFixtures}>
          ...
          <Suspense fallback={<TasksSkeleton />}>
            <TasksSection dealId={id} base="/v4/hub" isFixtures={isFixtures} />
          </Suspense>

          <DeleteDealButton dealId={id} dealName={deal.name} base="/v4/hub" isFixtures={isFixtures} />
        </ClientDetailShell>
      </div>
    </>
  );
