...
export default async function HubIntelDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <>
      <HubTopBar pageLabel="情報詳情" source={IS_DEMO ? 'fixtures' : 'supabase'} profile={profile} />
      <div className="mx-auto max-w-[1240px]">
        <IntelDetailV4 intel={intel} canEdit={canEdit} existingTags={tags ?? []} deals={deals ?? []} base="/hub" />
      </div>
    </>
  );
}
